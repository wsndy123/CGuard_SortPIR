...
namespace ourc3pai {
    ...
    class OurC3PaiSystem {
    public:
        // Tight default setting for 256-bit plaintexts:
        // use two 129-bit primes so n > 2^256, and the actual modulus n is typically 257 or 258 bits.
        explicit OurC3PaiSystem(int paillier_modulus_bits = 257);
        ~OurC3PaiSystem();

        OurC3PaiSystem(const OurC3PaiSystem&) = delete;
        OurC3PaiSystem& operator=(const OurC3PaiSystem&) = delete;
        ...
        std::size_t paillier_modulus_bits() const; // actual n bits
