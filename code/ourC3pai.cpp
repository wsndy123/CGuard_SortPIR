...
namespace ourc3pai {
    ...
    public:
        ...
        bool build_from_records(const std::vector<Record>& records, std::size_t preferred_rows)
        {
            ...
            mat_.resize(records_sorted_.size());
            ...
            for (std::size_t r = 0; r < rows_; ++r) {
                ...
                for (std::size_t c = 0; c < cols_; ++c) {
                    mat_[begin + c] = records_sorted_[begin + c].v;
                }
            }
        ...
        {
            if (!OurC3PaiSystem::ensure_parent_directories(path)) {
                return false;
            }
            ...
            const char magic[8] = { 'O', 'C', '3', 'P', 'A', 'I', '1', 0 };
            ofs.write(magic, 8);
        ...
        static bool load_records_binary(const std::string& path, std::vector<Record>* records)
        {
            ...
            const char expect[8] = { 'O', 'C', '3', 'P', 'A', 'I', '1', 0 };
    ...
    private:
            ...
            std::vector<BNPtr>* ans) const
        {
            ...
            for (std::size_t j = 0; j < cols_; ++j) {
                ...
                for (std::size_t i = 0; i < rows_; ++i) {
                    const U256& v = mat_[i * cols_ + j];
                    if (!BN_bin2bn(v.bytes.data(), static_cast<int>(U256_BYTES), exponent.get())) {
                        throw_openssl("BN_bin2bn(exponent)");
                    }
                    check_one(BN_mod_exp_mont(term.get(), qu[i].get(), exponent.get(),
                        paillier_.n_square(), ctx.get(), mont.get()),
                        "BN_mod_exp_mont(qu^v)");
        ...
        std::vector<U256> mat_;
        std::vector<std::uint64_t> row_first_keys_;
        std::vector<std::uint64_t> row_last_keys_;
        std::vector<std::uint64_t> flags_;
    };

    OurC3PaiSystem::OurC3PaiSystem(int paillier_modulus_bits)
        : impl_(new Impl(paillier_modulus_bits))
    {
    }

    OurC3PaiSystem::~OurC3PaiSystem()
    {
        delete impl_;
    }

    bool OurC3PaiSystem::build_from_records(const std::vector<Record>& records,
        std::size_t preferred_rows)
    {
        return impl_->build_from_records(records, preferred_rows);
    }

    QueryResult OurC3PaiSystem::query(std::uint64_t k, const U256& mu) const
    {
        return impl_->query(k, mu);
    }

    Dimensions OurC3PaiSystem::dims() const
    {
        return impl_->dims();
    }

    std::size_t OurC3PaiSystem::cipher_bytes() const
    {
        return impl_->cipher_bytes();
    }

    std::size_t OurC3PaiSystem::paillier_modulus_bits() const
    {
        return impl_->paillier_modulus_bits();
    }

    const std::vector<std::uint64_t>& OurC3PaiSystem::flags() const
    {
        return impl_->flags();
    }

    std::vector<Record> OurC3PaiSystem::generate_random_records(std::size_t N)
    {
        return Impl::generate_random_records(N);
    }

    bool OurC3PaiSystem::save_records_binary(const std::string& path,
        const std::vector<Record>& records)
    {
        return Impl::save_records_binary(path, records);
    }

    bool OurC3PaiSystem::load_records_binary(const std::string& path,
        std::vector<Record>* records)
    {
        return Impl::load_records_binary(path, records);
    }

    bool OurC3PaiSystem::ensure_parent_directories(const std::string& path)
    {
    ...
    {
        return to_fixed(value_mb, 6) + " MB";
    }

    std::string format_kb(double value_kb)
    {
        return to_fixed(value_kb, 6) + " KB";
    }
